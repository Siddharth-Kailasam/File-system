...
static int file_chmod(const char* oldpath,mode_t permissions)
{
	int chk=0,flag=0;
	printf("In chmod...\n");
	printf("Oldpath = %s\n",oldpath);
        ...
        struct directry*d=(struct directry*)malloc(sizeof(struct directry));
        flag = searchroot(oldpath,&f,&d,0);
        if(flag==-1)
        {
        	printf("Given pathway doesn't exist....\n");
        	return -ENOSYS;
        }
        struct file*ft = malloc(sizeof(struct file));
        if(flag==2&&strcmp(d->dname,strrchr(oldpath,'/')+1)==0)
        {
        	d->mode=S_IFDIR|permissions;
        	return 0;
        }
        if(flag==2)
        {
        	searchdir(d,f->fname,&ft);
        	f=ft;
        }
    	f->fil->mode=S_IFREG|permissions;
    	printf("chmod completed - %s.....\n",f->fname);
    	return 0;
}

static int file_truncate(const char* oldpath,off_t offset)
{
	if(offset<0)
		return -1;
	int chk=0,flag=0;
	...
	printf("Oldpath = %s\n",oldpath);
        ...
        if(offset>f->fil->size)
        {
        	printf("offset too large\n");
        	return -1;
        }
    	f->fil->offset=offset;
    	printf("trunc completed - %s.....\n",f->fname);
    	return 0;
}

//renames a file
static int file_rename(const char* oldpath,const char* newpath)
{
	int chk=0;
	char*temp1=(char*)malloc(sizeof(char)*strlen(oldpath)+1),*temp2=(char*)malloc(sizeof(char)*strlen(newpath)+1);
	strcpy(temp1,oldpath);
	...
	char* token = strtok(temp1, "/");  
        ...
        if(chk!=chk2)
        {
        	printf("Pathways don't match....\n");
        	return -ENOSYS;
        }
        printf("Oldpath = %s\n",oldpath);
        struct file*f=(struct file*)malloc(sizeof(struct file));
        struct directry*d=(struct directry*)malloc(sizeof(struct directry));
        int flag = searchroot(newpath,&f,&d,1);
        flag = searchroot(oldpath,&f,&d,0);
        if(flag==-1)
        {
        	printf("Given pathway doesn't exist....\n");
        ...
        {
        	strcpy(d->dname,strrchr(newpath,'/')+1);
        	return 0;
        }
        char *last = strrchr(newpath, '/');
       if (last != NULL) 
	{
    		strcpy(f->fname, last+1);
    		printf("Rename completed - %s.....\n",f->fname);
    		return 0;
	}
			 ...
			 off_t offset, struct fuse_file_info *fi)
{
	...
	{
		for(i=0;i<fileroot->filecount;i++)
			filler(buf,fileroot->files[i]->fname,NULL,0);
		
		struct directry *temp=(struct directry*)malloc(sizeof(struct directry));
		temp=fileroot->dir;
		for(i=0;i<fileroot->dircount;i++)
		{
			filler(buf,temp->dname,NULL,0);
			temp=temp->next;
		}
	}
...
static int file_read(const char *path, char *buf, size_t size,off_t offset, struct fuse_file_info *fi)
{
	...
	if(flag==1)
	{
		...
		{
			printf("Offset = %ld, file offset = %ld, size = %ld\n",offset,ft->fil->offset,size);
			...
			printf("Offset = %d\n",new_off);
			size_t avail = ft->fil->size - new_off; //available size
...
static int file_symlink(const char* oldpath, const char*newpath)
{
	printf("%s...\n",newpath);
	fflush(stdout);
	printf("%s...\n",oldpath);
	fflush(stdout);
	char*tok=strtok(oldpath,"/");
	...
	char*temp2;
	tok = strtok(newpath,"/");
	char* token1 = strtok(newpath, "/");
	char* fast = strtok(NULL,"/"); 
  	strcpy(temp2,token1);
    
    while (fast=strtok(NULL,"/")) { 
        //printf("%s\n", token);
        strcpy(fast,token1); 
        strcat(temp2,"/");
        strcat(temp2,token1);
	...
	struct file*f=(struct file*)malloc(sizeof(struct file));
	struct directry*d=(struct directry*)malloc(sizeof(struct directry));
	int flag=searchroot(oldpath,&f,&d,0);
	if(flag==-1)
	{
		printf("Invalid old pathway %s...\n",oldpath);
		return -errno;
	}
	if(flag==1)
	{
		struct directry*d1=(struct directry*)malloc(sizeof(struct directry));
		struct file*f1=(struct file*)malloc(sizeof(struct file));
		flag=searchroot(newpath,&f1,&d1,1);
		if(flag==-1)
		{
			printf("Invalid new pathway....\n");
			return -errno;
		}
		f1->fd_count=0;
		strcpy(f1->fname,strrchr(newpath,'/')+1);
		return 0;
	}
	flag=searchdir(d,oldpath,&f);
	if(flag==-1)
	{
		printf("Invalid old pathway....\n");
		return -errno;
	}
	struct directry*d1=(struct directry*)malloc(sizeof(struct directry));
	struct file*f1=(struct file*)malloc(sizeof(struct file));
	...
	f1->fd_count=0;
	f1->fil=f->fil;
	d1->f[d1->filecount]=f1;
	return 0;
}
//write in the file
static int file_write(const char *path, const char *buf, size_t size, off_t offset, struct fuse_file_info *fi) 
{
	if((int)strlen(buf)>=((int)super.totalfreeb-1)*(int)BLOCK_SIZE)
	{
		printf("File extremely large....\n");
		struct file*f=(struct file*)malloc(sizeof(struct file));
		struct directry*d=(struct directry*)malloc(sizeof(struct directry));
		int flag = searchroot(path,&f,&d,0);
		if(flag==-1)
			return -errno;
		if(flag==0)
			return -EDQUOT;
		...
		if(strcmp(strrchr(path,'/')+1,d->dname)==0)
			return -errno;
		flag = searchdir(d,strrchr(path,'/')+1,&f);
		if(flag==-1)
			return -errno;
		file_unlink(path);	
		return -EDQUOT;
...
void file_destroy(void *private_data)
{
	...
	int i,j;
	//Binary file
	d=fopen("/home/user/IOS project/file.txt","wb+");
...
int persistent()
{
	...
	d=fopen("/home/user/IOS project/file.txt","rb+");
